...
Adapts AW-ShortCut Flow from offline RL to RLPD-style online learning:
...
- Policy-Critic data separation for filtering failed samples
...
References:
- AW-ShortCut Flow: rlft/algorithms/offline_rl/aw_shortcut_flow.py
...
from rlft.networks import ShortCutVelocityUNet1D, EnsembleQNetwork, soft_update
...
class AWSCAgent(nn.Module):
    ...
    Key features:
    - EnsembleQNetwork for conservative Q estimates
    ...
    Args:
        ...
        q_network: EnsembleQNetwork for Q-value estimation (optional)
    ...
    def load_pretrained(
        ...
        Supports loading from both offline RL (AWShortCutFlowAgent) and 
        pure BC (ShortCutFlowAgent) checkpoints.
    ...
    def get_action(
        ...
        Returns full pred_horizon sequence for compatibility with AgentWrapper.
        
        Args:
            obs_features: (B, obs_dim) encoded observation features
            deterministic: Whether to disable exploration noise (ignored for flow)
    ...
    def compute_actor_loss(
        ...
        For online training where only act_horizon actions are available:
        - actions with shape (B, act_horizon, action_dim) will be automatically
          padded/extended to pred_horizon using policy sampling for BC loss,
          while keeping the original actions for Q-weighting.
        
        Args:
            obs_features: (B, obs_dim) observation features
            actions: (B, H, action_dim) expert actions for BC, H can be pred_horizon or act_horizon
        ...
        """
        # Handle action horizon mismatch: extend act_horizon actions to pred_horizon
        action_horizon = actions.shape[1]
        if action_horizon < self.pred_horizon:
            # For online data with only act_horizon actions, we compute BC loss
            # on the available horizon and use Q-weighting for the rest
            # Pad actions by repeating the last action or sampling from policy
            pad_size = self.pred_horizon - action_horizon
            # Simple padding: repeat last action (conservative approach)
            last_action = actions[:, -1:, :].expand(-1, pad_size, -1)
        ...
        with torch.no_grad():
            ...
            advantage_flat = advantage.squeeze(-1)
    ...
    def compute_critic_loss(
        ...
        q_values = self.critic(actions, obs_cond)
    ...
    def compute_loss(
        ...
        **kwargs,
        ...
        actor_loss, actor_metrics = self.compute_actor_loss(
            obs_features, actions, actions_for_q, is_demo
        )
