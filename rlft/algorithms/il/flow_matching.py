...
from rlft.networks import VelocityUNet1D
...
class FlowMatchingAgent(nn.Module):
    ...
    Args:
        ...
        action_bounds: Tuple (min, max) for action clipping, None to disable
    ...
    def get_action(
        ...
        obs_features: torch.Tensor,
        use_rk4: bool = False,
        **kwargs,  # Accept extra kwargs like deterministic for API compatibility
    ) -> torch.Tensor:
    ...
    def _rk4_integrate(self, x: torch.Tensor, obs_cond: torch.Tensor) -> torch.Tensor:
        ...
        for i in range(self.num_flow_steps):
            ...
            k2 = self.velocity_net(x + dt / 2 * k1, t_mid, global_cond=obs_cond)
            k3 = self.velocity_net(x + dt / 2 * k2, t_mid, global_cond=obs_cond)
            
            t_end = torch.full((B,), t + dt, device=device)
            k4 = self.velocity_net(x + dt * k3, t_end, global_cond=obs_cond)
    ...
    def get_action_deterministic(
        ...
        **kwargs,
        ...
        # Initialize from zero (deterministic)
        x = torch.zeros(batch_size, self.pred_horizon, self.action_dim, device=device)
