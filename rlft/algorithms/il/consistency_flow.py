...
Migrated from diffusion_policy framework to rlft.
...
from rlft.networks import VelocityUNet1D, soft_update
...
class ConsistencyFlowAgent(nn.Module):
    ...
    Args:
        ...
        cons_t_max: Maximum t for consistency (default: 0.95)
        cons_t_upper: Upper clamp for t_plus (default: 0.95)
        ...
        teacher_from: Teacher start point (default: "t_plus")
        student_point: Student evaluation point (default: "t_plus")
        consistency_loss_space: Loss space (default: "velocity")
        action_bounds: Tuple (min, max) for action clipping (default: None)
    ...
    def __init__(
        ...
        self.t_min = cons_t_min
        self.t_max = cons_t_max
        self.t_upper = cons_t_upper
        self.delta_min = cons_delta_min
        self.delta_max = cons_delta_max
    ...
    def compute_loss(
        ...
        total_loss = self.flow_weight * flow_loss + self.consistency_weight * consistency_loss
    ...
    def get_action(
        ...
        **kwargs,  # Accept extra kwargs for API compatibility
        ...
        """Generate action using flow ODE integration (stochastic from noise)."""
    ...
    def get_action_deterministic(
        ...
        **kwargs,
        ...
        """Generate action deterministically starting from zero (no noise)."""
        self.velocity_net.eval()
        batch_size = obs_features.shape[0]
        steps = num_steps if num_steps is not None else self.num_flow_steps
