"""
Environment Creation Utilities for ManiSkill.
"""
...
def make_eval_envs(
    ...
    For CPU vectorized environments only the first parallel environment is used 
    to record videos. For GPU vectorized environments all parallel environments 
    are used to record videos.

    Args:
        env_id: the environment id
        num_envs: the number of parallel environments
        sim_backend: the simulation backend to use. can be "physx_cpu" or "physx_cuda"
        env_kwargs: the environment kwargs. You can also pass in max_episode_steps 
                   to override the default max episode steps for the environment.
    ...
    """
    # Make a copy of env_kwargs to avoid modifying the original
    env_kwargs = dict(env_kwargs)
    
    # Set render_mode when video recording is enabled
    if video_dir and "render_mode" not in env_kwargs:
