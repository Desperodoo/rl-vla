...
参考: inference_rtc/tests/test_offline_inference.py
"""
...
class OfflineInferenceTest:
    ...
    def load_demo(self, traj_idx: int = 0) -> np.ndarray:
        ...
        Returns:
            ground_truth: (T, action_dim) 动作 ground truth
        """
    ...
    def predict(self, rgb: torch.Tensor, state: torch.Tensor) -> np.ndarray:
        ...
        Args:
            rgb: (B, T, C, H, W) 图像张量
            state: (B, T, state_dim) 状态张量
            
        Returns:
            first_action: (action_dim,) 第一个动作
        """
        ...
        # 取第一个动作 (obs_horizon - 1 是当前时刻对应的动作)
    ...
    def rollout(self, step_size: int = 1) -> np.ndarray:
        ...
        Args:
            step_size: 推理步长 (1 表示每帧都推理)
            
        Returns:
            predictions: (T, action_dim) 预测动作
        """
        print(f"\n[3] 进行轨迹推理 (step_size={step_size})...")
        
        rgb_all = self.processed_obs["rgb"]  # (T, C, H, W)
        state_all = self.processed_obs["state"]  # (T, state_dim)
    ...
    def compute_metrics(
        ...
        Args:
            predictions: (T, action_dim) 预测动作
            ground_truth: (T, action_dim) 真实动作
            
        Returns:
            metrics: 包含 mse, mae 等指标的字典
        """
        print(f"\n[4] 计算指标...")
        ...
        print(f"\n  Per-Dimension MAE:")
        dim_maes = {}
        for i, name in enumerate(action_names[:min(len(action_names), ground_truth.shape[1])]):
            dim_mae = np.mean(np.abs(predictions[:, i] - ground_truth[:, i]))
            dim_maes[name] = dim_mae
    ...
    def visualize(
        ...
        num_dims = min(action_dim, len(action_names))
        
        fig, axes = plt.subplots(num_dims, 1, figsize=(14, 2 * num_dims))
        if num_dims == 1:
            axes = [axes]
        
        timesteps = np.arange(T)
        
        for i, (ax, name) in enumerate(zip(axes, action_names[:num_dims])):
            ax.plot(timesteps, ground_truth[:, i], 'b-', label='Ground Truth', alpha=0.8, linewidth=1.5)
            ax.plot(timesteps, predictions[:, i], 'r--', label='Prediction', alpha=0.8, linewidth=1.5)
