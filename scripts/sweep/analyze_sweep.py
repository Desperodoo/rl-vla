#!/usr/bin/env python3
"""
Sweep Experiment Analysis Tool
==============================

This tool provides comprehensive analysis of hyperparameter sweep experiments:
1. Data extraction from training logs and TensorBoard events
2. Parameter sensitivity analysis (controlled variable analysis)
3. Training curve analysis (convergence, stability, overfitting)
4. Visualization (heatmaps, training curves, bar charts, radar plots)
5. Markdown report generation

Usage:
    python analyze_sweep.py --sweep-dir runs/maniskill_sweep --config-version v2
    python analyze_sweep.py --sweep-dir runs/maniskill_sweep --algorithm flow_matching
    python analyze_sweep.py --sweep-dir runs/maniskill_sweep --output-dir analysis_results
"""

import os
import re
import json
import argparse
from pathlib import Path
from datetime import datetime
from dataclasses import dataclass, field, asdict
from typing import Dict, List, Optional, Tuple, Any
from collections import defaultdict

import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import matplotlib.patches as mpatches
from matplotlib.gridspec import GridSpec

# Try to import tensorboard
try:
    from tensorboard.backend.event_processing import event_accumulator
    HAS_TENSORBOARD = True
except ImportError:
    HAS_TENSORBOARD = False
    print("Warning: tensorboard not installed. Training curve analysis will be limited.")

# =============================================================================
# Data Classes
# =============================================================================

@dataclass
class ExperimentResult:
    """Single experiment result"""
    algorithm: str
    config_name: str
    params: Dict[str, Any]  # Parsed hyperparameters
    success_once: Optional[float] = None
    success_at_end: Optional[float] = None
    status: str = "unknown"  # success, failed, not_started
    exp_dir: Optional[str] = None
    # Training metrics (time series)
    training_curves: Dict[str, List[Tuple[int, float]]] = field(default_factory=dict)
    # Final metrics
    final_metrics: Dict[str, float] = field(default_factory=dict)


@dataclass
class SweepAnalysis:
    """Analysis results for a sweep"""
    algorithm: str
    experiments: List[ExperimentResult]
    param_sensitivity: Dict[str, float] = field(default_factory=dict)
    best_config: Optional[str] = None
    best_score: float = 0.0
    param_values: Dict[str, List[Any]] = field(default_factory=dict)  # param -> unique values


# =============================================================================
# Configuration
# =============================================================================

# Default parameters for each algorithm (baseline)
DEFAULT_PARAMS = {
    "flow_matching": {
        "lr": 3e-4,
        "ema_decay": 0.999,
        "pred_horizon": 16,
        "obs_horizon": 2,
        "act_horizon": 8,
        "unet_dims": (64, 128, 256),
        "batch_size": 256,
        "num_flow_steps": 20,
    },
    "diffusion_policy": {
        "lr": 3e-4,
        "ema_decay": 0.999,
        "pred_horizon": 16,
        "obs_horizon": 2,
        "act_horizon": 8,
        "unet_dims": (64, 128, 256),
        "batch_size": 256,
        "num_diffusion_iters": 100,
    },
    "consistency_flow": {
        "lr": 3e-4,
        "ema_decay": 0.999,
        "bc_weight": 1.0,
        "consistency_weight": 1.0,
        "cons_delta_mode": "fixed",
        "cons_delta_fixed": 0.02,
        "cons_teacher_steps": 2,
    },
    "shortcut_flow": {
        "lr": 3e-4,
        "ema_decay": 0.999,
        "bc_weight": 1.0,
        "consistency_weight": 0.3,
        "sc_fixed_step_size": 0.125,
        "sc_self_consistency_k": 0.25,
    },
    "reflected_flow": {
        "lr": 3e-4,
        "ema_decay": 0.999,
        "reflection_mode": "hard",
        "boundary_reg_weight": 0.01,
        "num_flow_steps": 20,
    },
    "cpql": {
        "lr": 3e-4,
        "lr_critic": 3e-4,
        "bc_weight": 1.0,
        "consistency_weight": 0.3,
        "alpha": 0.001,
        "reward_scale": 0.1,
        "gamma": 0.99,
        "tau": 0.005,
    },
    "awcp": {
        "lr": 3e-4,
        "lr_critic": 3e-4,
        "bc_weight": 1.0,
        "consistency_weight": 0.3,
        "beta": 10.0,
        "weight_clip": 100.0,
        "reward_scale": 0.1,
    },
    "aw_shortcut_flow": {
        "lr": 3e-4,
        "lr_critic": 3e-4,
        "bc_weight": 1.0,
        "consistency_weight": 0.3,
        "beta": 10.0,
        "weight_clip": 100.0,
        "reward_scale": 0.1,
        "sc_fixed_step_size": 0.125,
    },
}

# Mapping from config name patterns to parameter changes
# Format: {pattern: {param: value}}
CONFIG_PARAM_PATTERNS = {
    # Learning rate
    r"lr_(\d+e-?\d+)": lambda m: {"lr": float(m.group(1))},
    r"lr_critic_(\d+e-?\d+)": lambda m: {"lr_critic": float(m.group(1))},
    
    # EMA
    r"ema_([\d.]+)": lambda m: {"ema_decay": float(m.group(1))},
    
    # Horizons
    r"pred_horizon_(\d+)": lambda m: {"pred_horizon": int(m.group(1))},
    r"obs_horizon_(\d+)": lambda m: {"obs_horizon": int(m.group(1))},
    r"act_horizon_(\d+)": lambda m: {"act_horizon": int(m.group(1))},
    
    # Architecture
    r"unet_small": lambda m: {"unet_dims": (32, 64, 128)},
    r"unet_large": lambda m: {"unet_dims": (128, 256, 512)},
    
    # Batch size
    r"batch_(\d+)": lambda m: {"batch_size": int(m.group(1))},
    
    # Flow/Diffusion steps
    r"num_flow_steps_(\d+)": lambda m: {"num_flow_steps": int(m.group(1))},
    r"flow_steps_(\d+)": lambda m: {"num_flow_steps": int(m.group(1))},
    r"steps_(\d+)": lambda m: {"num_flow_steps": int(m.group(1))},
    r"ddpm_steps_(\d+)": lambda m: {"num_diffusion_iters": int(m.group(1))},
    
    # Weights
    r"bc_weight_([\d.]+)": lambda m: {"bc_weight": float(m.group(1))},
    r"weights_([\d.]+)_([\d.]+)": lambda m: {"bc_weight": float(m.group(1)), "consistency_weight": float(m.group(2))},
    r"weights_flow_heavy": lambda m: {"bc_weight": 2.0, "consistency_weight": 0.5},
    r"weights_cons_heavy": lambda m: {"bc_weight": 0.5, "consistency_weight": 2.0},
    r"weights_equal": lambda m: {"bc_weight": 1.0, "consistency_weight": 1.0},
    r"weights_flow_only": lambda m: {"bc_weight": 1.0, "consistency_weight": 0.0},
    r"shortcut_weight_([\d.]+)": lambda m: {"consistency_weight": float(m.group(1))},
    
    # Consistency flow specific
    r"t_range_tight": lambda m: {"cons_t_min": 0.1, "cons_t_max": 0.9},
    r"t_range_wide": lambda m: {"cons_t_min": 0.02, "cons_t_max": 0.98},
    r"t_range_full": lambda m: {"cons_full_t_range": True},
    r"teacher_(\d+)step": lambda m: {"cons_teacher_steps": int(m.group(1))},
    r"delta_fixed_([\d.]+)": lambda m: {"cons_delta_mode": "fixed", "cons_delta_fixed": float(m.group(1))},
    r"delta_random_narrow": lambda m: {"cons_delta_mode": "random", "cons_delta_min": 0.01, "cons_delta_max": 0.05},
    r"delta_random_wide": lambda m: {"cons_delta_mode": "random", "cons_delta_min": 0.01, "cons_delta_max": 0.2},
    
    # Shortcut flow specific
    r"step_([\d.]+)": lambda m: {"sc_fixed_step_size": float(m.group(1))},
    r"sc_k_([\d.]+)": lambda m: {"sc_self_consistency_k": float(m.group(1))},
    r"step_uniform_small": lambda m: {"sc_step_size_mode": "uniform", "sc_min_step_size": 0.0625, "sc_max_step_size": 0.125},
    r"step_uniform_wide": lambda m: {"sc_step_size_mode": "uniform", "sc_min_step_size": 0.0625, "sc_max_step_size": 0.25},
    r"step_power2": lambda m: {"sc_step_size_mode": "power2"},
    r"step_uniform": lambda m: {"sc_step_size_mode": "uniform"},
    r"inference_(\d+)": lambda m: {"sc_num_inference_steps": int(m.group(1))},
    r"inference_adaptive": lambda m: {"sc_inference_mode": "adaptive"},
    r"target_endpoint": lambda m: {"sc_target_mode": "endpoint"},
    r"teacher_no_ema": lambda m: {"sc_use_ema_teacher": False},
    
    # Reflected flow specific
    r"reflection_soft": lambda m: {"reflection_mode": "soft"},
    r"boundary_reg_([\d.]+)": lambda m: {"boundary_reg_weight": float(m.group(1))},
    r"soft_reg_([\d.]+)": lambda m: {"reflection_mode": "soft", "boundary_reg_weight": float(m.group(1))},
    r"hard_steps_(\d+)": lambda m: {"reflection_mode": "hard", "num_flow_steps": int(m.group(1))},
    
    # Offline RL specific
    r"alpha_([\d.]+)": lambda m: {"alpha": float(m.group(1))},
    r"beta_([\d.]+)": lambda m: {"beta": float(m.group(1))},
    r"reward_scale_([\d.]+)": lambda m: {"reward_scale": float(m.group(1))},
    r"gamma_([\d.]+)": lambda m: {"gamma": float(m.group(1))},
    r"tau_([\d.]+)": lambda m: {"tau": float(m.group(1))},
    r"weight_clip_(\d+)": lambda m: {"weight_clip": float(m.group(1))},
    r"q_clip_([\d.]+)": lambda m: {"q_clip": float(m.group(1))},
    r"num_qs_(\d+)": lambda m: {"num_qs": int(m.group(1))},
    
    # Loss space
    r"loss_endpoint": lambda m: {"cons_loss_space": "endpoint"},
    
    # Combined configs (special handling)
    r"efficient_small": lambda m: {"num_diffusion_iters": 50, "unet_dims": (32, 64, 128)},
    r"efficient_medium": lambda m: {"num_diffusion_iters": 75, "unet_dims": (64, 128, 256)},
    r"combined_stable": lambda m: {"cons_delta_mode": "fixed", "cons_delta_fixed": 0.02, "cons_teacher_steps": 2, "ema_decay": 0.999},
    r"combined_fast": lambda m: {"cons_delta_mode": "fixed", "cons_delta_fixed": 0.05, "cons_teacher_steps": 1, "ema_decay": 0.995},
    r"stable_1step": lambda m: {"sc_fixed_step_size": 0.125, "sc_self_consistency_k": 0.25, "sc_teacher_steps": 1},
    r"aggressive_2step": lambda m: {"sc_fixed_step_size": 0.25, "sc_self_consistency_k": 0.5, "sc_teacher_steps": 2},
    r"stable": lambda m: {"config_type": "stable"},
    r"aggressive": lambda m: {"config_type": "aggressive"},
}


# =============================================================================
# Data Extraction
# =============================================================================

class DataExtractor:
    """Extract data from sweep experiments"""
    
    def __init__(self, sweep_dir: str, config_version: str = "v2"):
        self.sweep_dir = Path(sweep_dir)
        self.config_version = config_version
        
    def find_experiment_dirs(self, algorithm: str) -> List[Path]:
        """Find all experiment directories for an algorithm"""
        algo_dir = self.sweep_dir / algorithm
        if not algo_dir.exists():
            return []
        
        exp_dirs = []
        for item in algo_dir.iterdir():
            if item.is_dir():
                # Skip base directories (without timestamp)
                if "__" in item.name or not any(
                    (item / f).exists() for f in ["train.log", "checkpoints"]
                ):
                    # This is a timestamped directory or base dir without data
                    if "__" in item.name:
                        exp_dirs.append(item)
                    else:
                        # Check if there's a timestamped version
                        timestamped = list(algo_dir.glob(f"{item.name}__*"))
                        if timestamped:
                            exp_dirs.extend(timestamped)
                        elif (item / "train.log").exists():
                            exp_dirs.append(item)
        
        return sorted(set(exp_dirs))
    
    def get_config_name(self, exp_dir: Path) -> str:
        """Extract config name from experiment directory"""
        name = exp_dir.name
        # Remove timestamp suffix
        if "__" in name:
            name = name.rsplit("__", 1)[0]
        return name
    
    def parse_config_params(self, algorithm: str, config_name: str) -> Dict[str, Any]:
        """Parse hyperparameters from config name"""
        # Start with default params
        params = DEFAULT_PARAMS.get(algorithm, {}).copy()
        
        # Apply pattern matching
        for pattern, extractor in CONFIG_PARAM_PATTERNS.items():
            match = re.search(pattern, config_name)
            if match:
                try:
                    extracted = extractor(match)
                    params.update(extracted)
                except Exception:
                    pass
        
        return params
    
    def parse_metrics_from_log(self, log_file: Path) -> Dict[str, float]:
        """Parse metrics from train.log file"""
        metrics = {}
        
        if not log_file.exists():
            return metrics
        
        with open(log_file, 'r', errors='ignore') as f:
            content = f.read()
        
        # Parse wandb summary lines
        # Format: "wandb: metric_name value" or "wandb:    metric_name value"
        patterns = [
            (r"wandb:\s*final/best_success_once\s+([\d.]+)", "best_success_once"),
            (r"wandb:\s*final/best_success_at_end\s+([\d.]+)", "best_success_at_end"),
            (r"wandb:\s*eval/success_once\s+([\d.]+)", "final_success_once"),
            (r"wandb:\s*eval/success_at_end\s+([\d.]+)", "final_success_at_end"),
        ]
        
        for pattern, metric_name in patterns:
            matches = re.findall(pattern, content)
            if matches:
                # Take the last value (final summary)
                try:
                    value = float(matches[-1])
                    # Skip sparkline characters
                    if value <= 1.0:  # Valid probability
                        metrics[metric_name] = value
                except ValueError:
                    pass
        
        return metrics
    
    def load_tensorboard_data(self, exp_dir: Path) -> Dict[str, List[Tuple[int, float]]]:
        """Load training curves from TensorBoard event files"""
        curves = {}
        
        if not HAS_TENSORBOARD:
            return curves
        
        # Find event files
        event_files = list(exp_dir.glob("events.out.tfevents.*"))
        if not event_files:
            return curves
        
        try:
            ea = event_accumulator.EventAccumulator(str(exp_dir))
            ea.Reload()
            
            # Get all scalar tags
            tags = ea.Tags().get('scalars', [])
            
            for tag in tags:
                events = ea.Scalars(tag)
                curves[tag] = [(e.step, e.value) for e in events]
        except Exception as e:
            print(f"Warning: Failed to load TensorBoard data from {exp_dir}: {e}")
        
        return curves
    
    def extract_experiment(self, algorithm: str, exp_dir: Path) -> ExperimentResult:
        """Extract all data from a single experiment"""
        config_name = self.get_config_name(exp_dir)
        params = self.parse_config_params(algorithm, config_name)
        
        # Check status
        checkpoint_dir = exp_dir / "checkpoints"
        has_checkpoint = (
            (checkpoint_dir / "best_eval_success_once.pt").exists() or
            (checkpoint_dir / "final.pt").exists()
        )
        
        # Try to find log file (may be in base dir)
        log_file = exp_dir / "train.log"
        if not log_file.exists():
            base_dir = exp_dir.parent / config_name
            if (base_dir / "train.log").exists():
                log_file = base_dir / "train.log"
        
        # Parse metrics from log
        metrics = self.parse_metrics_from_log(log_file)
        
        # Determine status
        if has_checkpoint or metrics.get("best_success_once") is not None:
            status = "success"
        elif log_file.exists():
            status = "failed"
        else:
            status = "not_started"
        
        # Load training curves
        training_curves = self.load_tensorboard_data(exp_dir)
        
        return ExperimentResult(
            algorithm=algorithm,
            config_name=config_name,
            params=params,
            success_once=metrics.get("best_success_once"),
            success_at_end=metrics.get("best_success_at_end"),
            status=status,
            exp_dir=str(exp_dir),
            training_curves=training_curves,
            final_metrics=metrics,
        )
    
    def extract_algorithm(self, algorithm: str) -> List[ExperimentResult]:
        """Extract all experiments for an algorithm"""
        exp_dirs = self.find_experiment_dirs(algorithm)
        results = []
        
        for exp_dir in exp_dirs:
            result = self.extract_experiment(algorithm, exp_dir)
            results.append(result)
        
        return results


# =============================================================================
# Parameter Sensitivity Analysis
# =============================================================================

class SensitivityAnalyzer:
    """Analyze parameter sensitivity using controlled variable analysis"""
    
    def __init__(self, experiments: List[ExperimentResult]):
        self.experiments = [e for e in experiments if e.success_once is not None]
        self.df = self._build_dataframe()
    
    def _build_dataframe(self) -> pd.DataFrame:
        """Build a DataFrame from experiments"""
        data = []
        for exp in self.experiments:
            row = {
                "config_name": exp.config_name,
                "success_once": exp.success_once,
                "success_at_end": exp.success_at_end,
                **exp.params
            }
            data.append(row)
        return pd.DataFrame(data)
    
    def analyze_single_param(self, param: str) -> Dict[str, Any]:
        """Analyze the effect of a single parameter"""
        if param not in self.df.columns:
            return {}
        
        # Group by parameter value
        grouped = self.df.groupby(param)["success_once"]
        
        # Calculate statistics
        stats = grouped.agg(["mean", "std", "min", "max", "count"]).reset_index()
        
        # Calculate sensitivity (range of means)
        if len(stats) > 1:
            sensitivity = stats["mean"].max() - stats["mean"].min()
        else:
            sensitivity = 0.0
        
        # Find best value
        best_idx = stats["mean"].idxmax()
        best_value = stats.loc[best_idx, param]
        best_mean = stats.loc[best_idx, "mean"]
        
        return {
            "param": param,
            "sensitivity": sensitivity,
            "best_value": best_value,
            "best_mean": best_mean,
            "values": stats[param].tolist(),
            "means": stats["mean"].tolist(),
            "stds": stats["std"].fillna(0).tolist(),
            "counts": stats["count"].tolist(),
            "stats_df": stats,
        }
    
    def analyze_all_params(self) -> Dict[str, Dict[str, Any]]:
        """Analyze sensitivity for all parameters"""
        results = {}
        
        # Get all parameter columns (exclude non-param columns)
        exclude = {"config_name", "success_once", "success_at_end", "config_type"}
        param_cols = [c for c in self.df.columns if c not in exclude]
        
        for param in param_cols:
            # Skip if only one unique value
            unique_values = self.df[param].dropna().unique()
            if len(unique_values) <= 1:
                continue
            
            # Skip if values are not comparable (e.g., tuples)
            if isinstance(unique_values[0], (tuple, list)):
                # Convert to string for comparison
                self.df[f"{param}_str"] = self.df[param].apply(str)
                analysis = self.analyze_single_param(f"{param}_str")
                if analysis:
                    analysis["param"] = param
                    results[param] = analysis
            else:
                analysis = self.analyze_single_param(param)
                if analysis:
                    results[param] = analysis
        
        return results
    
    def get_sensitivity_ranking(self) -> List[Tuple[str, float]]:
        """Get parameters ranked by sensitivity"""
        analyses = self.analyze_all_params()
        ranking = [(p, a["sensitivity"]) for p, a in analyses.items()]
        return sorted(ranking, key=lambda x: x[1], reverse=True)
    
    def get_best_params(self) -> Dict[str, Any]:
        """Get the best value for each parameter"""
        analyses = self.analyze_all_params()
        return {p: a["best_value"] for p, a in analyses.items()}


# =============================================================================
# Training Curve Analysis
# =============================================================================

class TrainingCurveAnalyzer:
    """Analyze training curves for convergence, stability, etc."""
    
    def __init__(self, experiments: List[ExperimentResult]):
        self.experiments = experiments
    
    def get_curve(self, exp: ExperimentResult, metric: str) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        """Get a training curve as numpy arrays"""
        if metric not in exp.training_curves:
            return None
        
        data = exp.training_curves[metric]
        if not data:
            return None
        
        steps = np.array([d[0] for d in data])
        values = np.array([d[1] for d in data])
        return steps, values
    
    def analyze_convergence(self, exp: ExperimentResult, metric: str = "train/loss") -> Dict[str, float]:
        """Analyze convergence characteristics of a training curve"""
        curve = self.get_curve(exp, metric)
        if curve is None:
            return {}
        
        steps, values = curve
        
        # Final value
        final_value = values[-1] if len(values) > 0 else np.nan
        
        # Convergence speed (steps to reach 90% of final improvement)
        if len(values) > 1:
            initial = values[0]
            final = values[-1]
            threshold = initial - 0.9 * (initial - final)
            conv_idx = np.where(values <= threshold)[0]
            conv_steps = steps[conv_idx[0]] if len(conv_idx) > 0 else steps[-1]
        else:
            conv_steps = np.nan
        
        # Stability (std of last 20% of training)
        last_portion = int(len(values) * 0.2)
        if last_portion > 1:
            stability = np.std(values[-last_portion:])
        else:
            stability = np.nan
        
        # Monotonicity (fraction of steps where loss decreased)
        if len(values) > 1:
            diffs = np.diff(values)
            monotonicity = np.mean(diffs < 0)
        else:
            monotonicity = np.nan
        
        return {
            "final_value": final_value,
            "convergence_steps": conv_steps,
            "stability": stability,
            "monotonicity": monotonicity,
        }
    
    def compare_curves(self, metric: str, configs: Optional[List[str]] = None) -> pd.DataFrame:
        """Compare training curves across experiments"""
        data = []
        
        for exp in self.experiments:
            if configs and exp.config_name not in configs:
                continue
            
            analysis = self.analyze_convergence(exp, metric)
            if analysis:
                row = {
                    "config_name": exp.config_name,
                    "success_once": exp.success_once,
                    **analysis
                }
                data.append(row)
        
        return pd.DataFrame(data)


# =============================================================================
# Visualization
# =============================================================================

class SweepVisualizer:
    """Generate visualizations for sweep analysis"""
    
    def __init__(self, output_dir: str = "analysis_results"):
        self.base_output_dir = Path(output_dir)
        self.base_output_dir.mkdir(parents=True, exist_ok=True)
        self.output_dir = self.base_output_dir  # Current output directory
        
        # Set style
        plt.style.use('seaborn-v0_8-whitegrid')
        plt.rcParams['figure.figsize'] = (12, 8)
        plt.rcParams['font.size'] = 10
        plt.rcParams['axes.titlesize'] = 12
        plt.rcParams['axes.labelsize'] = 10
    
    def set_algorithm_dir(self, algorithm: str):
        """Set output directory for a specific algorithm"""
        self.output_dir = self.base_output_dir / algorithm
        self.output_dir.mkdir(parents=True, exist_ok=True)
    
    def reset_to_base_dir(self):
        """Reset output directory to base directory"""
        self.output_dir = self.base_output_dir
    
    def plot_param_sensitivity_bar(self, sensitivity_data: Dict[str, Dict], 
                                    algorithm: str, top_n: int = 15) -> str:
        """Plot parameter sensitivity as horizontal bar chart"""
        fig, ax = plt.subplots(figsize=(10, 8))
        
        # Sort by sensitivity
        sorted_params = sorted(
            sensitivity_data.items(), 
            key=lambda x: x[1]["sensitivity"], 
            reverse=True
        )[:top_n]
        
        params = [p[0] for p in sorted_params]
        sensitivities = [p[1]["sensitivity"] for p in sorted_params]
        
        # Create horizontal bar chart
        colors = plt.cm.RdYlGn(np.linspace(0.8, 0.2, len(params)))
        bars = ax.barh(params, sensitivities, color=colors)
        
        ax.set_xlabel("Sensitivity (Δ success_once)")
        ax.set_title(f"Parameter Sensitivity Analysis - {algorithm}")
        ax.invert_yaxis()
        
        # Add value labels
        for bar, val in zip(bars, sensitivities):
            ax.text(val + 0.005, bar.get_y() + bar.get_height()/2, 
                   f"{val:.3f}", va='center', fontsize=9)
        
        plt.tight_layout()
        
        filepath = self.output_dir / "param_sensitivity.png"
        plt.savefig(filepath, dpi=150, bbox_inches='tight')
        plt.close()
        
        return str(filepath)
    
    def plot_param_effect(self, sensitivity_data: Dict[str, Dict], 
                          param: str, algorithm: str) -> str:
        """Plot the effect of a single parameter on success rate"""
        if param not in sensitivity_data:
            return ""
        
        data = sensitivity_data[param]
        
        fig, ax = plt.subplots(figsize=(10, 6))
        
        values = data["values"]
        means = data["means"]
        stds = data["stds"]
        
        # Convert values to strings for categorical axis
        x_labels = [str(v) for v in values]
        x_pos = np.arange(len(values))
        
        bars = ax.bar(x_pos, means, yerr=stds, capsize=5, 
                     color=plt.cm.Blues(0.6), edgecolor='navy', alpha=0.8)
        
        ax.set_xlabel(param)
        ax.set_ylabel("success_once")
        ax.set_title(f"Effect of {param} on Success Rate - {algorithm}")
        ax.set_xticks(x_pos)
        ax.set_xticklabels(x_labels, rotation=45, ha='right')
        
        # Highlight best value
        best_idx = means.index(max(means))
        bars[best_idx].set_color('green')
        bars[best_idx].set_alpha(1.0)
        
        # Add value labels
        for bar, mean in zip(bars, means):
            ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.01,
                   f"{mean:.3f}", ha='center', va='bottom', fontsize=9)
        
        plt.tight_layout()
        
        filepath = self.output_dir / f"{param}_effect.png"
        plt.savefig(filepath, dpi=150, bbox_inches='tight')
        plt.close()
        
        return str(filepath)
    
    def plot_algorithm_comparison(self, all_analyses: Dict[str, SweepAnalysis]) -> str:
        """Plot comparison of best results across algorithms"""
        fig, ax = plt.subplots(figsize=(12, 6))
        
        algorithms = []
        best_scores = []
        colors = []
        
        # Color map for algorithm types
        algo_colors = {
            "flow_matching": "#2ecc71",
            "diffusion_policy": "#3498db",
            "consistency_flow": "#9b59b6",
            "shortcut_flow": "#e74c3c",
            "reflected_flow": "#f39c12",
            "cpql": "#1abc9c",
            "awcp": "#e91e63",
            "aw_shortcut_flow": "#ff5722",
        }
        
        for algo, analysis in all_analyses.items():
            if analysis.best_score > 0:
                algorithms.append(algo)
                best_scores.append(analysis.best_score)
                colors.append(algo_colors.get(algo, "#95a5a6"))
        
        # Sort by score
        sorted_indices = np.argsort(best_scores)[::-1]
        algorithms = [algorithms[i] for i in sorted_indices]
        best_scores = [best_scores[i] for i in sorted_indices]
        colors = [colors[i] for i in sorted_indices]
        
        bars = ax.bar(algorithms, best_scores, color=colors, edgecolor='black', alpha=0.8)
        
        ax.set_xlabel("Algorithm")
        ax.set_ylabel("Best success_once")
        ax.set_title("Algorithm Comparison - Best Performance")
        ax.set_ylim(0, 1.0)
        
        # Rotate x labels
        plt.xticks(rotation=45, ha='right')
        
        # Add value labels
        for bar, score in zip(bars, best_scores):
            ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.02,
                   f"{score:.2f}", ha='center', va='bottom', fontsize=10, fontweight='bold')
        
        # Add legend for algorithm types
        il_patch = mpatches.Patch(color='#3498db', label='Imitation Learning')
        rl_patch = mpatches.Patch(color='#e91e63', label='Offline RL')
        ax.legend(handles=[il_patch, rl_patch], loc='upper right')
        
        plt.tight_layout()
        
        filepath = self.output_dir / "algorithm_comparison.png"
        plt.savefig(filepath, dpi=150, bbox_inches='tight')
        plt.close()
        
        return str(filepath)
    
    def plot_training_curves(self, experiments: List[ExperimentResult], 
                             metric: str, algorithm: str,
                             top_n: int = 5, metric_label: Optional[str] = None) -> str:
        """Plot training curves for top N experiments"""
        # Sort by success_once and take top N
        sorted_exps = sorted(
            [e for e in experiments if e.success_once is not None],
            key=lambda x: x.success_once,
            reverse=True
        )[:top_n]
        
        # Check if any experiment has this metric
        has_data = False
        for exp in sorted_exps:
            if metric in exp.training_curves and exp.training_curves[metric]:
                has_data = True
                break
        
        if not has_data:
            return ""
        
        fig, ax = plt.subplots(figsize=(12, 6))
        colors = plt.cm.viridis(np.linspace(0, 0.8, len(sorted_exps)))
        
        plotted = False
        for exp, color in zip(sorted_exps, colors):
            if metric not in exp.training_curves:
                continue
            
            data = exp.training_curves[metric]
            if not data:
                continue
            
            steps = [d[0] for d in data]
            values = [d[1] for d in data]
            
            label = f"{exp.config_name} ({exp.success_once:.2f})"
            ax.plot(steps, values, label=label, color=color, linewidth=1.5)
            plotted = True
        
        if not plotted:
            plt.close()
            return ""
        
        ax.set_xlabel("Training Steps")
        ax.set_ylabel(metric_label or metric)
        
        # Determine filename based on metric
        if "loss" in metric.lower():
            filename = "training_loss_curves.png"
            ax.set_title(f"Training Loss Curves - {algorithm} (Top {top_n})")
        elif "success" in metric.lower():
            filename = "eval_success_curves.png"
            ax.set_title(f"Evaluation Success Rate - {algorithm} (Top {top_n})")
        else:
            filename = f"{metric.replace('/', '_')}_curves.png"
            ax.set_title(f"{metric} - {algorithm} (Top {top_n})")
        
        ax.legend(loc='best', fontsize=8)
        plt.tight_layout()
        
        filepath = self.output_dir / filename
        plt.savefig(filepath, dpi=150, bbox_inches='tight')
        plt.close()
        
        return str(filepath)
    
    def plot_heatmap(self, df: pd.DataFrame, x_param: str, y_param: str, 
                     algorithm: str) -> str:
        """Plot heatmap of success rate for two parameters"""
        if x_param not in df.columns or y_param not in df.columns:
            return ""
        
        # Pivot table
        pivot = df.pivot_table(
            values="success_once", 
            index=y_param, 
            columns=x_param, 
            aggfunc="mean"
        )
        
        if pivot.empty:
            return ""
        
        fig, ax = plt.subplots(figsize=(10, 8))
        
        im = ax.imshow(pivot.values, cmap='RdYlGn', aspect='auto')
        
        # Set ticks
        ax.set_xticks(np.arange(len(pivot.columns)))
        ax.set_yticks(np.arange(len(pivot.index)))
        ax.set_xticklabels([str(v) for v in pivot.columns], rotation=45, ha='right')
        ax.set_yticklabels([str(v) for v in pivot.index])
        
        ax.set_xlabel(x_param)
        ax.set_ylabel(y_param)
        ax.set_title(f"Success Rate Heatmap - {algorithm}")
        
        # Add colorbar
        cbar = plt.colorbar(im, ax=ax)
        cbar.set_label("success_once")
        
        # Add value annotations
        for i in range(len(pivot.index)):
            for j in range(len(pivot.columns)):
                val = pivot.iloc[i, j]
                if not np.isnan(val):
                    ax.text(j, i, f"{val:.2f}", ha='center', va='center', 
                           color='black' if val > 0.5 else 'white', fontsize=9)
        
        plt.tight_layout()
        
        filepath = self.output_dir / f"{x_param}_vs_{y_param}_heatmap.png"
        plt.savefig(filepath, dpi=150, bbox_inches='tight')
        plt.close()
        
        return str(filepath)
    
    def plot_config_ranking(self, experiments: List[ExperimentResult], 
                            algorithm: str, top_n: int = 20) -> str:
        """Plot ranking of all configurations"""
        fig, ax = plt.subplots(figsize=(12, max(8, top_n * 0.4)))
        
        # Sort by success_once
        sorted_exps = sorted(
            [e for e in experiments if e.success_once is not None],
            key=lambda x: x.success_once,
            reverse=True
        )[:top_n]
        
        configs = [e.config_name for e in sorted_exps]
        scores = [e.success_once for e in sorted_exps]
        
        # Color gradient
        colors = plt.cm.RdYlGn(np.linspace(0.8, 0.2, len(configs)))
        
        bars = ax.barh(configs, scores, color=colors, edgecolor='black', alpha=0.8)
        
        ax.set_xlabel("success_once")
        ax.set_title(f"Configuration Ranking - {algorithm} (Top {top_n})")
        ax.invert_yaxis()
        ax.set_xlim(0, 1.0)
        
        # Add value labels
        for bar, score in zip(bars, scores):
            ax.text(score + 0.01, bar.get_y() + bar.get_height()/2,
                   f"{score:.3f}", va='center', fontsize=9)
        
        plt.tight_layout()
        
        filepath = self.output_dir / "config_ranking.png"
        plt.savefig(filepath, dpi=150, bbox_inches='tight')
        plt.close()
        
        return str(filepath)


# =============================================================================
# Report Generation
# =============================================================================

class ReportGenerator:
    """Generate Markdown analysis report"""
    
    def __init__(self, output_dir: str = "analysis_results"):
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(parents=True, exist_ok=True)
    
    def generate_algorithm_report(self, analysis: SweepAnalysis, 
                                   sensitivity_data: Dict[str, Dict],
                                   figure_paths: Dict[str, str]) -> str:
        """Generate report section for one algorithm"""
        lines = []
        
        lines.append(f"## {analysis.algorithm}")
        lines.append("")
        
        # Summary
        total = len(analysis.experiments)
        completed = len([e for e in analysis.experiments if e.status == "success"])
        lines.append(f"**Experiments**: {completed}/{total} completed")
        lines.append(f"**Best Config**: `{analysis.best_config}` (success_once = {analysis.best_score:.3f})")
        lines.append("")
        
        # Parameter sensitivity ranking
        if sensitivity_data:
            lines.append("### Parameter Sensitivity Ranking")
            lines.append("")
            lines.append("| Rank | Parameter | Sensitivity | Best Value | Best Mean |")
            lines.append("|------|-----------|-------------|------------|-----------|")
            
            sorted_params = sorted(
                sensitivity_data.items(),
                key=lambda x: x[1]["sensitivity"],
                reverse=True
            )
            
            for rank, (param, data) in enumerate(sorted_params[:10], 1):
                sens = data["sensitivity"]
                best_val = data["best_value"]
                best_mean = data["best_mean"]
                lines.append(f"| {rank} | `{param}` | {sens:.4f} | {best_val} | {best_mean:.3f} |")
            
            lines.append("")
        
        # Figures - use relative paths from report location
        if "sensitivity" in figure_paths:
            lines.append("### Parameter Sensitivity")
            # Get relative path: algorithm/filename.png
            rel_path = f"{analysis.algorithm}/{Path(figure_paths['sensitivity']).name}"
            lines.append(f"![Sensitivity]({rel_path})")
            lines.append("")
        
        if "ranking" in figure_paths:
            lines.append("### Configuration Ranking")
            rel_path = f"{analysis.algorithm}/{Path(figure_paths['ranking']).name}"
            lines.append(f"![Ranking]({rel_path})")
            lines.append("")
        
        if "curves" in figure_paths:
            lines.append("### Training Loss Curves")
            rel_path = f"{analysis.algorithm}/{Path(figure_paths['curves']).name}"
            lines.append(f"![Training Curves]({rel_path})")
            lines.append("")
        
        if "eval_curves" in figure_paths:
            lines.append("### Evaluation Success Curves")
            rel_path = f"{analysis.algorithm}/{Path(figure_paths['eval_curves']).name}"
            lines.append(f"![Eval Curves]({rel_path})")
            lines.append("")
        
        # Key findings
        lines.append("### Key Findings")
        lines.append("")
        
        if sensitivity_data:
            # Most impactful parameters
            top_params = sorted(
                sensitivity_data.items(),
                key=lambda x: x[1]["sensitivity"],
                reverse=True
            )[:3]
            
            for param, data in top_params:
                lines.append(f"- **{param}**: Best value = `{data['best_value']}` "
                           f"(mean = {data['best_mean']:.3f}, sensitivity = {data['sensitivity']:.3f})")
        
        lines.append("")
        return "\n".join(lines)
    
    def generate_full_report(self, all_analyses: Dict[str, SweepAnalysis],
                             all_sensitivity: Dict[str, Dict],
                             all_figures: Dict[str, Dict[str, str]]) -> str:
        """Generate full analysis report"""
        lines = []
        
        # Header
        lines.append("# Sweep Experiment Analysis Report")
        lines.append("")
        lines.append(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        lines.append("")
        
        # Executive Summary
        lines.append("## Executive Summary")
        lines.append("")
        
        # Overall best
        best_algo = None
        best_score = 0
        best_config = None
        
        for algo, analysis in all_analyses.items():
            if analysis.best_score > best_score:
                best_score = analysis.best_score
                best_algo = algo
                best_config = analysis.best_config
        
        if best_algo:
            lines.append(f"**Overall Best**: `{best_algo}` / `{best_config}` with success_once = **{best_score:.3f}**")
            lines.append("")
        
        # Algorithm ranking
        lines.append("### Algorithm Performance Ranking")
        lines.append("")
        lines.append("| Rank | Algorithm | Best success_once | Best Config |")
        lines.append("|------|-----------|-------------------|-------------|")
        
        sorted_algos = sorted(
            all_analyses.items(),
            key=lambda x: x[1].best_score,
            reverse=True
        )
        
        for rank, (algo, analysis) in enumerate(sorted_algos, 1):
            lines.append(f"| {rank} | `{algo}` | {analysis.best_score:.3f} | `{analysis.best_config}` |")
        
        lines.append("")
        
        # Algorithm comparison figure
        if "comparison" in all_figures:
            lines.append("![Algorithm Comparison](algorithm_comparison.png)")
            lines.append("")
        
        # Per-algorithm sections
        for algo in sorted_algos:
            algo_name = algo[0]
            if algo_name in all_analyses:
                figures = all_figures.get(algo_name, {})
                sensitivity = all_sensitivity.get(algo_name, {})
                lines.append(self.generate_algorithm_report(
                    all_analyses[algo_name], sensitivity, figures
                ))
        
        # Recommendations
        lines.append("## Recommendations")
        lines.append("")
        lines.append("Based on the analysis, here are the recommended configurations:")
        lines.append("")
        
        for algo, analysis in sorted_algos[:3]:
            if analysis.best_config:
                lines.append(f"### {algo}")
                lines.append(f"- Use config: `{analysis.best_config}`")
                
                # Get best params from sensitivity
                if algo in all_sensitivity:
                    sens = all_sensitivity[algo]
                    top_params = sorted(sens.items(), key=lambda x: x[1]["sensitivity"], reverse=True)[:5]
                    for param, data in top_params:
                        lines.append(f"- `{param}`: {data['best_value']}")
                lines.append("")
        
        return "\n".join(lines)
    
    def save_report(self, content: str, filename: str = "analysis_report.md"):
        """Save report to file"""
        filepath = self.output_dir / filename
        with open(filepath, 'w') as f:
            f.write(content)
        return str(filepath)


# =============================================================================
# Main Analysis Pipeline
# =============================================================================

class SweepAnalyzer:
    """Main analysis pipeline"""
    
    def __init__(self, sweep_dir: str, config_version: str = "v2", 
                 output_dir: str = "analysis_results"):
        self.sweep_dir = Path(sweep_dir)
        self.config_version = config_version
        self.output_dir = Path(output_dir)
        
        self.extractor = DataExtractor(sweep_dir, config_version)
        self.visualizer = SweepVisualizer(output_dir)
        self.report_gen = ReportGenerator(output_dir)
        
        self.algorithms = [
            "flow_matching", "diffusion_policy", "consistency_flow",
            "shortcut_flow", "reflected_flow", "cpql", "awcp", "aw_shortcut_flow"
        ]
    
    def analyze_algorithm(self, algorithm: str) -> Tuple[SweepAnalysis, Dict, Dict[str, str]]:
        """Run full analysis for one algorithm"""
        print(f"Analyzing {algorithm}...")
        
        # Set output directory for this algorithm
        self.visualizer.set_algorithm_dir(algorithm)
        
        # Extract data
        experiments = self.extractor.extract_algorithm(algorithm)
        if not experiments:
            print(f"  No experiments found for {algorithm}")
            return None, {}, {}
        
        print(f"  Found {len(experiments)} experiments")
        
        # Build analysis object
        analysis = SweepAnalysis(
            algorithm=algorithm,
            experiments=experiments,
        )
        
        # Find best
        completed = [e for e in experiments if e.success_once is not None]
        if completed:
            best_exp = max(completed, key=lambda x: x.success_once)
            analysis.best_config = best_exp.config_name
            analysis.best_score = best_exp.success_once
        
        # Sensitivity analysis
        sensitivity_data = {}
        if len(completed) >= 2:
            analyzer = SensitivityAnalyzer(completed)
            sensitivity_data = analyzer.analyze_all_params()
            analysis.param_sensitivity = {p: d["sensitivity"] for p, d in sensitivity_data.items()}
        
        # Generate figures
        figures = {}
        
        if sensitivity_data:
            figures["sensitivity"] = self.visualizer.plot_param_sensitivity_bar(
                sensitivity_data, algorithm
            )
            
            # Plot top 3 most sensitive parameters
            top_params = sorted(sensitivity_data.items(), 
                               key=lambda x: x[1]["sensitivity"], reverse=True)[:3]
            for param, _ in top_params:
                fig_path = self.visualizer.plot_param_effect(sensitivity_data, param, algorithm)
                if fig_path:
                    figures[f"effect_{param}"] = fig_path
        
        if completed:
            figures["ranking"] = self.visualizer.plot_config_ranking(experiments, algorithm)
            
            # Training curves (if available)
            has_curves = any(exp.training_curves for exp in experiments)
            if has_curves:
                # Try multiple metric names (different algorithms may use different names)
                for metric in ["losses/loss", "losses/flow_loss", "losses/diffusion_loss", "train/loss"]:
                    curves_path = self.visualizer.plot_training_curves(
                        experiments, metric, algorithm
                    )
                    if curves_path:
                        figures["curves"] = curves_path
                        break
                
                # Also plot eval/success_once curve
                eval_curves_path = self.visualizer.plot_training_curves(
                    experiments, "eval/success_once", algorithm, metric_label="Success Rate"
                )
                if eval_curves_path:
                    figures["eval_curves"] = eval_curves_path
        
        return analysis, sensitivity_data, figures
    
    def analyze_all(self, algorithms: Optional[List[str]] = None) -> str:
        """Run full analysis for all algorithms"""
        if algorithms is None:
            algorithms = self.algorithms
        
        all_analyses = {}
        all_sensitivity = {}
        all_figures = {"comparison": None}
        
        for algo in algorithms:
            result = self.analyze_algorithm(algo)
            if result[0] is not None:
                all_analyses[algo] = result[0]
                all_sensitivity[algo] = result[1]
                all_figures[algo] = result[2]
        
        # Reset to base directory for global figures
        self.visualizer.reset_to_base_dir()
        
        # Generate comparison figure
        if len(all_analyses) >= 2:
            all_figures["comparison"] = self.visualizer.plot_algorithm_comparison(all_analyses)
        
        # Generate report
        report = self.report_gen.generate_full_report(
            all_analyses, all_sensitivity, all_figures
        )
        report_path = self.report_gen.save_report(report)
        
        # Save raw data as JSON
        self.save_raw_data(all_analyses, all_sensitivity)
        
        print(f"\nAnalysis complete!")
        print(f"Report saved to: {report_path}")
        print(f"Figures saved to: {self.output_dir}")
        
        return report_path
    
    def save_raw_data(self, all_analyses: Dict[str, SweepAnalysis], 
                      all_sensitivity: Dict[str, Dict]):
        """Save raw analysis data as JSON"""
        
        def convert_to_json_serializable(obj):
            """Convert numpy types to Python native types"""
            if isinstance(obj, (np.integer, np.int64, np.int32)):
                return int(obj)
            elif isinstance(obj, (np.floating, np.float64, np.float32)):
                return float(obj)
            elif isinstance(obj, np.ndarray):
                return obj.tolist()
            elif isinstance(obj, tuple):
                return str(obj)
            elif isinstance(obj, dict):
                return {k: convert_to_json_serializable(v) for k, v in obj.items()}
            elif isinstance(obj, list):
                return [convert_to_json_serializable(v) for v in obj]
            return obj
        
        data = {
            "timestamp": datetime.now().isoformat(),
            "config_version": self.config_version,
            "algorithms": {}
        }
        
        for algo, analysis in all_analyses.items():
            algo_data = {
                "best_config": analysis.best_config,
                "best_score": convert_to_json_serializable(analysis.best_score),
                "experiments": []
            }
            
            for exp in analysis.experiments:
                exp_data = {
                    "config_name": exp.config_name,
                    "success_once": convert_to_json_serializable(exp.success_once),
                    "success_at_end": convert_to_json_serializable(exp.success_at_end),
                    "status": exp.status,
                    "params": {k: convert_to_json_serializable(v) 
                             for k, v in exp.params.items()},
                }
                algo_data["experiments"].append(exp_data)
            
            # Add sensitivity data
            if algo in all_sensitivity:
                algo_data["sensitivity"] = {
                    param: {
                        "sensitivity": convert_to_json_serializable(d["sensitivity"]),
                        "best_value": convert_to_json_serializable(d["best_value"]),
                        "best_mean": convert_to_json_serializable(d["best_mean"]),
                    }
                    for param, d in all_sensitivity[algo].items()
                }
            
            data["algorithms"][algo] = algo_data
        
        filepath = self.output_dir / "analysis_data.json"
        with open(filepath, 'w') as f:
            json.dump(data, f, indent=2)
        
        print(f"Raw data saved to: {filepath}")


# =============================================================================
# CLI
# =============================================================================

def main():
    parser = argparse.ArgumentParser(
        description="Analyze hyperparameter sweep experiments",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__
    )
    
    parser.add_argument(
        "--sweep-dir", "-d",
        default=None,
        help="Directory containing sweep experiments (default: auto by config-version)"
    )
    
    parser.add_argument(
        "--config-version", "-v",
        default="v2",
        choices=["v1", "v2", "v3"],
        help="Config version (v1=wave1, v2=wave2, v3=wave3)"
    )
    
    parser.add_argument(
        "--algorithm", "-a",
        help="Analyze only specific algorithm"
    )
    
    parser.add_argument(
        "--output-dir", "-o",
        default=None,
        help="Output directory for results (default: <sweep-dir>/analysis_results)"
    )
    
    parser.add_argument(
        "--no-curves",
        action="store_true",
        help="Skip training curve analysis (faster)"
    )
    
    args = parser.parse_args()
    
    # Set sweep-dir based on config-version if not specified
    if args.sweep_dir is None:
        if args.config_version == "v3":
            args.sweep_dir = "runs/maniskill_sweep_v3"
        else:
            args.sweep_dir = "runs/maniskill_sweep"
    
    # Set output-dir based on sweep-dir if not specified
    if args.output_dir is None:
        args.output_dir = os.path.join(args.sweep_dir, "analysis_results")
    
    # Run analysis
    analyzer = SweepAnalyzer(
        sweep_dir=args.sweep_dir,
        config_version=args.config_version,
        output_dir=args.output_dir
    )
    
    if args.algorithm:
        analyzer.analyze_all([args.algorithm])
    else:
        analyzer.analyze_all()


if __name__ == "__main__":
    main()
