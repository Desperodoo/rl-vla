#!/bin/bash
# =============================================================================
# Data-Efficiency Comparison — Utilities
# =============================================================================
# Adapted from sweep_dsrl/utils.sh and sweep_pld/utils.sh.
# Key difference: run_experiment() dispatches to three different training
# scripts (train_rlpd, train_pld, train_dsrl) based on algorithm type.
# =============================================================================
...
# Activate conda environment (rlft_ms3) and set PYTHONPATH
if [[ -z "${_COMPARE_ENV_INIT:-}" ]]; then
    source "$(conda info --base)/etc/profile.d/conda.sh" 2>/dev/null || true
    conda activate rlft_ms3 2>/dev/null || true
    export PYTHONPATH="${PROJECT_ROOT}:${PYTHONPATH:-}"
    export _COMPARE_ENV_INIT=1
fi
...
get_checkpoint_path() {
    ...
    elif [[ -f "${exp_dir}/checkpoints/best.pt" ]]; then
...
is_experiment_successful() {
    ...
    if [[ -f "$log_file" ]]; then
        if grep -qE "Training completed|Saving final checkpoint|All training runs finished" "$log_file" 2>/dev/null; then
            return 0
        fi
    fi
...
parse_metrics_from_log() {
    ...
    local value
    value=$(grep -P "wandb:\s+${metric_name}\s" "$log_file" 2>/dev/null | tail -1 | awk '{print $NF}')
...
parse_best_success_rate() {
    ...
    # 1. wandb summary: final/best_success_rate
    ...
    # 3. eval/success_once (fallback)
    value=$(grep -P 'wandb:\s+eval/success_once\s' "$log_file" 2>/dev/null | tail -1 | awk '{print $NF}')
    if [[ -n "$value" ]] && [[ "$value" != "▁" ]]; then
        echo "$value"
        return
    fi
...
check_demo_file() {
    local demo_path="${DEMO_PATH/#\~/$HOME}"
    if [[ ! -f "$demo_path" ]]; then
        log_error "Demo file not found: ${DEMO_PATH}"
        log_info "AWSC requires demo data. Set DEMO_PATH env var."
        return 1
    fi
    log_info "Demo file: ${DEMO_PATH}"
    return 0
}

# -----------------------------------------------------------------------------
# Build Training Command per Algorithm Type
# -----------------------------------------------------------------------------
# Dispatches to the correct training script with per-algorithm base args.
# extra_args (from SWEEP_CONFIGS) override these defaults.
# -----------------------------------------------------------------------------
build_train_command() {
    local gpu_id=$1
    local algorithm=$2
    ...
    if [[ "${USE_WANDB}" == "true" ]]; then
        case "$algorithm" in
            awsc) wandb_args="--track --wandb_project_name ${WANDB_PROJECT}" ;;
    ...
    local cmd="CUDA_VISIBLE_DEVICES=${gpu_id}"

    # Per-algorithm eval frequency
    local algo_eval_freq
    case "$algorithm" in
        awsc) algo_eval_freq="${EVAL_FREQ_AWSC:-${EVAL_FREQ}}" ;;
        pld)  algo_eval_freq="${EVAL_FREQ_PLD:-${EVAL_FREQ}}" ;;
        dsrl) algo_eval_freq="${EVAL_FREQ_DSRL:-${EVAL_FREQ}}" ;;
        *)    algo_eval_freq="${EVAL_FREQ}" ;;
    esac

    case "$algorithm" in
        awsc)
            cmd+=" python -m rlft.online.train_rlpd"
            cmd+=" --algorithm awsc"
            cmd+=" --env_id ${ENV_ID}"
        ...
        pld)
            ...
            cmd+=" --num_eval_envs ${NUM_EVAL_ENVS}"
            cmd+=" --batch_size ${BATCH_SIZE}"
            cmd+=" --total_timesteps ${TOTAL_STEPS_PLD}"
            cmd+=" --eval_freq ${algo_eval_freq}"
            ...
            ;;
        dsrl)
            cmd+=" python -m rlft.online.train_dsrl"
            cmd+=" --env_id ${ENV_ID}"
            cmd+=" --obs_mode ${OBS_MODE}"
            cmd+=" --control_mode ${CONTROL_MODE}"
            cmd+=" --sim_backend ${SIM_BACKEND}"
            ...
            cmd+=" --num_eval_envs ${NUM_EVAL_ENVS}"
            cmd+=" --batch_size ${BATCH_SIZE}"
            cmd+=" --total_timesteps ${TOTAL_STEPS_DSRL}"
            cmd+=" --eval_freq ${algo_eval_freq}"
            cmd+=" --save_freq ${SAVE_FREQ}"
            cmd+=" --num_eval_episodes ${NUM_EVAL_EPISODES}"
            cmd+=" --seed ${SEED}"
    ...
    # Append per-config overrides (extra_args override the defaults above)
...
run_experiment() {
    ...
    local cmd
    cmd=$(build_train_command "$gpu_id" "$algorithm" "$config_name" "$extra_args")
...
# -----------------------------------------------------------------------------
# Global Queue Scheduling — Fill GPUs across all algorithms
# Identical logic to sweep_dsrl/pld queue mode.
# -----------------------------------------------------------------------------
run_sweep_queue() {
    local items=("$@")
    local total=${#items[@]}
    ...
    log_info "Running ${total} configs (GPUs: ${#AVAILABLE_GPUS[@]})"
...
load_algorithm_configs() {
    ...
    local config_file="${SCRIPT_DIR}/${config_dir}/fair_comparison.sh"
    ...
    # Select per-algorithm config array
    local -n configs_ref="${algorithm^^}_CONFIGS"
    if [[ ${#configs_ref[@]} -eq 0 ]]; then
        log_error "No configs found for algorithm: ${algorithm}"
        return 1
    fi

    SWEEP_CONFIGS=("${configs_ref[@]}")
    echo "${SWEEP_CONFIGS[@]}"
}
...
find_failed_experiments() {
    ...
    load_algorithm_configs "$algorithm" >/dev/null 2>&1 || return
...
# -----------------------------------------------------------------------------
# Analyze Results with Metrics (detailed)
# -----------------------------------------------------------------------------
analyze_algorithm_with_metrics() {
    ...
    local results=()
    local total=0 completed=0 failed=0 not_started=0
    ...
    local best_config="" best_score="0"
...
# -----------------------------------------------------------------------------
export_results_json() {
    local output_file=${1:-"${SWEEP_BASE_DIR}/comparison_results.json"}

    echo "{" > "$output_file"
    echo '  "timestamp": "'$(date -Iseconds)'",' >> "$output_file"
    echo '  "comparison_scheme": "'${COMPARISON_SCHEME}'",' >> "$output_file"
    echo '  "env_id": "'${ENV_ID}'",' >> "$output_file"
    echo '  "checkpoint": "'${CHECKPOINT}'",' >> "$output_file"
    echo '  "seed": '${SEED}',' >> "$output_file"
    echo '  "total_steps": {"awsc": '${TOTAL_STEPS_AWSC}', "pld": '${TOTAL_STEPS_PLD}', "dsrl": '${TOTAL_STEPS_DSRL}'},' >> "$output_file"
    echo '  "algorithms": {' >> "$output_file"

    local first_algo=true
    local global_best_config="" global_best_score="0" global_best_algo=""

    for algorithm in "${ALL_ALGORITHMS[@]}"; do
        load_algorithm_configs "$algorithm" >/dev/null 2>&1 || continue
        ...
        local algo_best_config="" algo_best_score="0"

        echo -n '    "'${algorithm}'": {"configs": [' >> "$output_file"
        ...
        for config in "${SWEEP_CONFIGS[@]}"; do
            ...
            local status="not_started" success_once="" success_end=""
