#!/bin/bash
# =============================================================================
# Sweep Utilities — Helper functions for PLD hyperparameter sweep
# =============================================================================
...
# train_pld.py creates dirs like: runs/{exp_name}__{timestamp}
find_actual_exp_dir() {
    local algorithm=$1
    local config_name=$2
...
is_experiment_successful() {
    ...
    # Best checkpoint (train_pld.py saves best.pt)
...
# Parse best success rate from train_pld.py log output
...
# Parse success_at_end from train_pld.py log
...
check_checkpoint() {
    ...
    if [[ ! -f "$ckpt_path" ]]; then
        ...
        log_info "PLD requires a pretrained ShortCut Flow checkpoint."
...
run_experiment() {
    local gpu_id=$1
    local algorithm=$2       # always "pld_sac" but kept for directory structure
    local config_name=$3
    local extra_args=$4
    ...
    # Build command — python -m rlft.online.train_pld
    local cmd="CUDA_VISIBLE_DEVICES=${gpu_id} python -m rlft.online.train_pld"
    ...
    # PLD-specific defaults from config.sh
    cmd+=" --action_scale ${ACTION_SCALE}"
    cmd+=" --calql_pretrain_steps ${CALQL_PRETRAIN_STEPS}"
    cmd+=" --calql_alpha ${CALQL_ALPHA}"
    cmd+=" --offline_demo_episodes ${OFFLINE_DEMO_EPISODES}"
    cmd+=" --probe_steps ${PROBE_STEPS}"
    cmd+=" --probing_alpha ${PROBING_ALPHA}"
    cmd+=" --online_ratio ${ONLINE_RATIO}"
    cmd+=" --online_buffer_size ${ONLINE_BUFFER_SIZE}"
    cmd+=" --offline_buffer_size ${OFFLINE_BUFFER_SIZE}"

    # Experiment naming
    cmd+=" --exp_name ${EXP_NAME}/${algorithm}/${config_name}"
    cmd+=" ${wandb_args}"

    # Per-config overrides (extra_args override the defaults above)
    cmd+=" ${extra_args}"
...
# -----------------------------------------------------------------------------
export_results_json() {
    local output_file=${1:-"sweep_pld_results.json"}
    local config_dir
    config_dir=$(get_config_dir)
    ...
    echo '  "pld_defaults": {' >> "$output_file"
    echo '    "action_scale": '${ACTION_SCALE}',' >> "$output_file"
    echo '    "calql_pretrain_steps": '${CALQL_PRETRAIN_STEPS}',' >> "$output_file"
    echo '    "calql_alpha": '${CALQL_ALPHA}',' >> "$output_file"
    echo '    "offline_demo_episodes": '${OFFLINE_DEMO_EPISODES}',' >> "$output_file"
    echo '    "probe_steps": '${PROBE_STEPS}',' >> "$output_file"
    echo '    "probing_alpha": '${PROBING_ALPHA}',' >> "$output_file"
    echo '    "online_ratio": '${ONLINE_RATIO} >> "$output_file"
    echo '  },' >> "$output_file"
