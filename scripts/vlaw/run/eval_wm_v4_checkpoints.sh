#!/bin/bash
# Dense checkpoint evaluation of WM v4 (iter1_v4) across GPUs 2-9
# Evaluates all 20 checkpoints (step 200 to 4000) in 3 rounds of 8 parallel jobs
# Usage: bash scripts/vlaw/run/eval_wm_v4_checkpoints.sh [--round 1|2|3|all]
...
launch_round() {
    ...
    for i in "${!steps[@]}"; do
        local step=${steps[$i]}
        local gpu=${GPUS[$i]}
        local out_dir="$OUT_BASE/v4_eval_step${step}"
        local log_file="$LOG_BASE/v4_eval_step${step}.log"
...
# Clear PID file
echo "# WM v4 Imagination Eval — $(date)" > "$PID_FILE"
...
case "$ROUND_NUM" in
    ...
    all)
        ...
        echo "  ls $OUT_BASE/v4_eval_step*/"
        echo "  ls $OUT_BASE/v4_eval_step*/viz/"
        echo "=========================================="
        ;;
    *)
