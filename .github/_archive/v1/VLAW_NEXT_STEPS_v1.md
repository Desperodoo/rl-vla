...
> **最后更新**: 2026-03-03 20:00 (T+7) | **历史版本**: `docs/vlaw/archive/VLAW_NEXT_STEPS_*.md`
...
- ⏸️ **ADR-024: WM 相关任务全部阻塞** — 等待 T-TIMING-QUICKTEST 解决. 方案 A (num_interact=4) 不影响现有 WM ckpt; 方案 B (frame_skip=4) 需数据+WM 推倒重来
...
## � T-TIMING-FIX: 帧率与 Imagination 时长修正 (ADR-023 + ADR-024 Gating)

> **调研报告**: [`docs/vlaw/frame_rate_timing_analysis.md`](../docs/vlaw/frame_rate_timing_analysis.md)
> **核心发现**: 当前 Imagination 生成 60 帧 (9 秒), 但 LiftPegUpright 中位数仅需 16 帧 (2.4 秒). ~73% 帧为无效外推, 持续退化拖低 VLM 评分.
> **ADR-024 阻塞**: 所有 WM 相关任务 (WM 消融/评估/imagination/迭代) 全部挂起, 直到 T-TIMING-QUICKTEST 完成.
...
|--------|---------|------|-------|------|
| **P0** | **T-TIMING-QUICKTEST** | **方案 A Gating: num_interact=4 生成 50 条 → VLM 标注 → D_syn+ 通过率** | Imagination+Reward | ⬜ 待执行 |
| **P0** | **T-EXP-VLM-03-BUG** | **排查 VLM 步数消融 p_yes≈0 的 bug** | Reward | ⬜ 待排查 |
| **P0** | **T-EXP-VLM-02-EVAL** | **r=32, r=64 补齐 ROC-AUC + Youden 评估** | Reward | ⬜ 待执行 |
| **P0** | **T-BC-FLYWHEEL-EVAL** | **A vs B 正式对比报告** | Eval | ⬜ 待分析 |
...
| P1 | **T-TIMING-A-FULL** | **方案 A 全量: num_interact=4 生成 200 条** | Imagination+Reward | ⬜ (T-TIMING-QUICKTEST Go) |
| P1 | **T-TIMING-FRAMESKIP** | **方案 B: frame_skip=4 对齐 5Hz + 数据+WM 推倒重来** | Data+WM | ⬜ (方案 A 验证有效) |
| P1 | ~~**T-BC-FLYWHEEL-A**~~ | ~~A 组 (纯 demo) 100K步 BC 基线~~ | ~~Policy~~ | ✅ 100d=0.10, 669d=0.54 |
| P1 | **T-BC-FLYWHEEL-B** | **B 组 (demo + D_syn+) 100K步训练** | Policy | ✅ 113t=0.34(+240%), 682t=0.48(-11%) |
| P1 | T-BC-FLYWHEEL-EVAL | A vs B 对比评估 | Eval | ⬜ 等 B 完成 |
...
| 支线 | **T-EXP-WM-05v2** | **WM 最优步数搜索 v2** (num_frames=5, demos, 20 ckpts) | WM | ⏸️ 训练完成, 评估阻塞 (ADR-024) |
| 支线 | T-EXP-WM-02~04 | WM 扩展消融 | WM | ⏸️ BLOCKED (ADR-024) |
| 支线 | **T-EXP-VLM-02** | VLM LoRA rank 消融 | Reward | ✅ r=8/r=32/r=64 全完成, r=32/r=64 缺 AUC |
| 支线 | **T-EXP-VLM-03** | VLM 步数消融 | Reward | ✅ 100/400/800步全完成, ⚠️ p_yes≈0 异常, 待 bug 排查 |
...
### E1: WM 配置消融 (GPU 0-3) ⏸️ BLOCKED (ADR-024)

> **阻塞原因**: 所有 WM 相关实验等待 TIMING 解决后恢复
